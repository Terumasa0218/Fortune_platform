import type { Planet, WesternReading, ZodiacSign } from "./western-types";
...
const SUN_SIGN_INTERPRETATION: Record<ZodiacSign, SignInterpretation> = {
  ...
  ふたご: {
    ...
    loveStyle: "知的な刺激を求める恋愛スタイル。会話が合う相手には深く惹かれる。",
  },
  かに: {
  ...
  しし: {
    ...
    loveStyle: "愛情表現が豊かで情熱的。褒められることで燃え上がり、パートナーを大切に扱う。",
  },
  おとめ: {
    ...
    loveStyle: "慎重に相手を見極めてから心を開く。言葉より行動で愛を示すタイプ。",
  },
  てんびん: {
    ...
    loveStyle: "ロマンティックで理想の愛を求める。対等なパートナーシップを大切にする。",
  },
  さそり: {
  ...
  いて: {
    ...
    loveStyle: "束縛を嫌い、精神的な自由を求める。一緒に冒険できるパートナーを探している。",
  },
  やぎ: {
    ...
    loveStyle: "恋愛にも真剣で長期的な関係を重視。責任感が強く、パートナーを全力でサポートする。",
  },
  みずがめ: {
    ...
    loveStyle: "精神的なつながりを重視。友達のような関係から始まる恋愛が理想。",
  },
  うお: {
    ...
    loveStyle: "ロマンティックで理想の愛を信じる。魂レベルで融け合うような深い愛を求める。",
  },
};
...
const MOON_SIGN_SUPPLEMENT: Record<ZodiacSign, string> = {
  ...
  てんびん: "感情のバランスを保つことを重視する。",
  さそり: "感情の深さは計り知れず、傷つくと長く引きずる。",
  いて: "楽観的な感情の持ち主で、切り替えが早い。",
  やぎ: "感情を内に秘め、弱みを見せることが苦手。",
  みずがめ: "感情を客観視する癖があり、冷静に見える。",
  うお: "感受性が非常に高く、他者の感情を自分のことのように感じる。",
};

export function interpretWesternChart(params: {
  sunSign: ZodiacSign;
  moonSign: ZodiacSign;
  planets: Planet[];
}): WesternReading {
  const base = SUN_SIGN_INTERPRETATION[params.sunSign];
  const moonSupplement =
    params.sunSign !== params.moonSign
      ? `月は${params.moonSign}座：${MOON_SIGN_SUPPLEMENT[params.moonSign]}`
      : "";
